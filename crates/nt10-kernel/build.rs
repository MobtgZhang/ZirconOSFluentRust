...
fn main() {
    ...
    let rs_wp = format!(
        r#"// @generated by ZFOS kernel build script (build.rs)
...
"#
    ...
    let mut icon_rs = String::from(
        "// @generated by ZFOS kernel build script (build.rs) — Start menu + Explorer + desktop shortcuts (BGRA8)\n",
    );
    ...
    let ui_font: BuildUiFont = match find_ui_sans_bytes(&root) {
        Some(bytes) => {
            match fontdue::Font::from_bytes(bytes, fontdue::FontSettings::default()) {
                ...
                Err(_) => {
                    println!("cargo:warning=ZFOS build: fontdue rejected UI font bytes — using placeholders");
        ...
        None => {
            ...
            println!("cargo:warning=ZFOS build: no OFL UI font found — placeholder text rasters (run ./scripts/fetch-ofl-fonts.sh)");
            BuildUiFont::Placeholder
        }
    };
    ...
    let mut ascii_rs = String::from("// @generated by ZFOS kernel build script (build.rs) — 8×8 bitmask rows for ASCII 32..=126 (OFL font or placeholder)\n");
